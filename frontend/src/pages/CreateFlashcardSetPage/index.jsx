...
const GROUP_MAPPING = {
  "Gia đình & Quan hệ": { tags: ["giadinh", "connguoi"], icon: "👨‍👩‍👧" },
  "Cuộc sống & Nhà cửa": {
    tags: ["nhacua", "dovan", "dodung", "vesinh"],
    icon: "🏠",
  },
  "Ăn uống & Thực phẩm": {
    tags: ["doan", "thit", "rau", "luongthuc", "giavi"],
    icon: "🍲",
  },
  "Động vật & Thiên nhiên": {
    tags: ["dongvat", "thiennhien", "mua", "thoitiet"],
    icon: "🦁",
  },
  "Thời gian": { tags: ["thoigian"], icon: "⏰" },
  "Địa điểm & Phương hướng": {
    tags: ["diadiem", "thanhpho", "vitri", "huong", "kientruc"],
    icon: "📍",
  },
  "Giao thông & Du lịch": { tags: ["giaothong"], icon: "✈️" },
  "Hành động (Động từ)": { tags: ["dongtu"], icon: "🏃" },
  "Tính chất (Tính từ)": {
    tags: ["tinhtu", "tinhtu_i", "tinhtu_na", "trangthai"],
    icon: "🎨",
  },
  "Trường học & Công việc": {
    tags: ["hoc_tap", "truonghoc", "giaoduc", "nghenghiep", "congty", "it"],
    icon: "🎓",
  },
  "Đại từ & Từ để hỏi": {
    tags: ["daitu", "nghevan", "tunghevan", "tu_noi"],
    icon: "❓",
  },
  "Màu sắc & Đồ vật & Khác": {
    tags: [
...
const CreateFlashcardSetPage = () => {
  ...
  // 1. Hiển thị mặc định chủ đề đầu tiên
  const [selectedTopic, setSelectedTopic] = useState(
    Object.keys(GROUP_MAPPING)[0]
  );
  ...
  useEffect(() => {
    if (savedVocabularies.length === 0) {
      setSelectedItems((prev) => prev.filter((item) => item.type !== "camera"));
      return;
    }
    const validCameraIds = new Set(savedVocabularies.map((v) => v.id));
    setSelectedItems((prev) =>
      prev.filter(
        (item) => item.type !== "camera" || validCameraIds.has(item.id)
      )
    );
  }, [savedVocabularies]);
    ...
    if (activeTab === "library" && selectedTopic) {
      const fetchSystemWords = async () => {
        ...
        try {
          ...
          const topicList = topicData ? topicData.tags : [];
  ...
  // Kiểm tra xem tất cả từ trong topic hiện tại đã được chọn chưa
  ...
  const handleToggleAllFromTopic = () => {
    if (isAllTopicSelected) {
      // Bỏ chọn tất cả từ thuộc topic này
      ...
      // Chọn tất cả (tránh trùng lặp)
      setSelectedItems((prev) => {
        const merged = [...prev];
        systemVocabs.forEach((v) => {
          if (
            !merged.some((item) => item.type === "system" && item.id === v.id)
          ) {
    ...
    <div className="min-h-screen bg-[#FAFAFA] pb-32">
      ...
      <div className="sticky top-0 z-20 bg-[#FEE9E7] pt-8 pb-4 px-5 rounded-b-[32px] shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          <button
            onClick={() => navigate(-1)}
            className="w-10 h-10 rounded-2xl bg-white flex items-center justify-center text-[#4A4A4A] shadow-sm active:scale-95 transition-transform"
          >
            <ChevronLeft size={24} />
          </button>
        ...
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex justify-between items-end">
            ...
            {activeTab === "library" && systemVocabs.length > 0 && (
              <button
                onClick={handleToggleAllFromTopic}
              ...
              >
                ...
                {isAllTopicSelected
                  ? "Bỏ chọn tất cả"
                  : `Chọn tất cả ${systemVocabs.length} từ`}
              </button>
            )}
          </div>
          ...
          {activeTab === "library" && (
            <button
              ...
              className="flex items-center justify-between w-full bg-white border border-gray-100 rounded-2xl px-5 py-4 shadow-sm active:scale-[0.98] transition-all"
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-[#FEE9E7] rounded-xl flex items-center justify-center text-[#E85A4F]">
                  <Filter size={20} />
                </div>
          ...
          {activeTab === "camera" ? (
            cameraLoading ? (
              ...
              savedVocabularies.map((v) => (
                <VocabularyCard
                  key={v.id}
                  word={v.vocabulary_details?.word}
                  meaning={v.vocabulary_details?.meaning}
                  image={v.user_image || v.vocabulary_details?.image_url}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white/90 to-transparent pointer-events-none">
        <button
          ...
          disabled={selectedItems.length < 5 || !formData.name.trim()}
          className="pointer-events-auto w-full py-4 bg-[#E85A4F] text-white rounded-2xl font-extrabold text-lg shadow-[0_8px_25px_-5px_rgba(232,90,79,0.5)] active:scale-[0.98] transition-all disabled:opacity-40 flex items-center justify-center gap-2"
        >
...
const TabButton = ({ active, onClick, icon, label }) => (
  <button
    ...
    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all ${
      active ? "bg-white text-[#E85A4F] shadow-sm" : "text-gray-500"
    }`}
  >
    {icon} {label}
  </button>
...
const VocabularyCard = ({ word, meaning, image, selected, onToggle }) => (
  <div
    onClick={onToggle}
    className={`flex items-center gap-4 p-3 rounded-[20px] transition-all duration-300 border ${
      selected
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <div className="font-bold text-[#2D2D2D] text-base truncate">
        {word || "???"}
      </div>
      <div className="text-gray-400 text-xs font-medium truncate mt-0.5">
        {meaning || "???"}
      </div>
    </div>
    <div
...
const EmptyState = ({ msg }) => (
  <div className="text-center py-12 bg-white rounded-[32px] border-2 border-dashed border-gray-100">
    ...
    <p className="text-gray-400 text-sm font-medium px-10 leading-relaxed">
      {msg}
    </p>
  </div>
);
