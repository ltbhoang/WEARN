...
export const useDataStore = create((set, get) => ({
    ...
    fetchCollections: async (force = false) => {
        ...
        if (!force && get().collections.length > 0 && (now - get().lastFetched.collections) < CACHE_TTL) {
            console.log('🟢 Dùng cache collections');
            return;
        }
        set(state => ({ loadingStates: { ...state.loadingStates, collections: true }, error: null }));
        try {
            ...
            set({ error: "Không thể lấy danh sách bộ sưu tập", loadingStates: { ...get().loadingStates, collections: false } });
        }
    },
    ...
    fetchCollectionDetail: async (id, force = false) => {
        ...
        set(state => ({
            loadingStates: { ...state.loadingStates, detail: { ...state.loadingStates.detail, [id]: true } },
            error: null
        }));
        try {
            ...
            set(state => ({
                vocabulariesByCollection: { ...state.vocabulariesByCollection, [id]: vocabularies },
                loadingStates: { ...state.loadingStates, detail: { ...state.loadingStates.detail, [id]: false } }
            }));
            return response.data;
        } catch (err) {
            set({
                error: err.message,
                loadingStates: { ...get().loadingStates, detail: { ...get().loadingStates.detail, [id]: false } }
            });
        }
    },
    ...
    fetchVocabulariesByTopic: async (topic, force = false) => {
        ...
        set(state => ({
            loadingStates: { ...state.loadingStates, topics: { ...state.loadingStates.topics, [topic]: true } },
            error: null
        }));
        try {
            const response = await axiosPrivate.get(`/api/vocabularies/?topic=${topic}`);
            const data = response.data;
            set(state => ({
                vocabulariesByTopic: { ...state.vocabulariesByTopic, [topic]: data },
                lastFetched: { ...state.lastFetched, topics: { ...state.lastFetched.topics, [topic]: now } },
                loadingStates: { ...state.loadingStates, topics: { ...state.loadingStates.topics, [topic]: false } }
            }));
            return data;
        } catch (err) {
            set({
                error: err.message,
                loadingStates: { ...get().loadingStates, topics: { ...get().loadingStates.topics, [topic]: false } }
            });
            return [];
        }
    },
    ...
    addVocabulary: async (vocabData) => {
        try {
            ...
            const currentCollectionVocabs = get().vocabulariesByCollection[collectionId] || [];
            set({
                vocabulariesByCollection: {
    ...
    fetchStreak: async (force = false) => {
        ...
        if (!force && get().streakData && (now - get().lastFetched.streak) < CACHE_TTL) {
            console.log('🟢 Dùng cache streak');
            return get().streakData;
        }
        set(state => ({ loadingStates: { ...state.loadingStates, streak: true }, error: null }));
        try {
    ...
    resetCache: () => {
        set({
            ...
            loadingStates: { collections: false, detail: {}, topics: {}, streak: false },
            error: null
        });
    }
}));